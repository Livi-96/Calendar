...
export default function Calendar() {
  ...
  function collums(row) {
    //function to check if there are any events on the day being rendered - returns an array of events that day
    ...
    for (let i = 0; i < 7; i++) {
      ...
      //month day represents the day of the month being rendered 
      const monthDay = i + row * 7 + (-firstDay + 1);
      //checks is the day being rendered is todays date
      if (monthDay == currentDate.getDate() && viewableDate.getMonth() == currentDate.getMonth() && viewableDate.getFullYear() == currentDate.getFullYear()) {
        //returns a coloured button to indicate it is the current day and checks if there are any events to be rendered
        ...
        columnElements.push(
          ...
          >
            <Text>{monthDay}</Text>
            {/* flatlist will render any events for the day if there are any */}
            <FlatList
              data={checkEventsToday(monthDay)}
              renderItem={({ item, index: number }) => {
                let img = null
                if(item.event == 'Predicted end of period'){
                  img = dottedDroplet
                } else {img = Droplet} return (
                <Pressable
                  style={styles.eventMarker}
                  onPress={() => handleEventClick(eventsList.indexOf(item))}
                >
                  <Image source={img} style={styles.flowImage}></Image>
                </Pressable>
              )}}
            />
          </Pressable>
      ...
      <View style={styles.container}>
        ...
        >
          <FlowForm
            ...
            dateObj={new Date(viewableDate.getFullYear(),viewableDate.getMonth(),clickedDay)}
            setModalOpen={setaddEventOpen}
