      ...
      <View style={styles.modalContainer}>
        ...
        <Formik
          ...
          onSubmit={(values) => {
            ...
            //set some condition to check what kind of event has been added so can add predicted dates for period/cycle timings based off added event
            switch (dropDown) {
              case "Start of period":
                //  logic to add date for predicted period end and cycle end
                let periodEnd = new Date(dateObj);
                periodEnd.setDate(periodEnd.getDate() + 7);
                //  let cycleEnd = new Date(dateObj);
                //  cycleEnd.setDate(cycleEnd.getDate() + predictedPeriodLength);
                setEventslist((curr) => [
                  ...curr,
                  values,
                  {
                    event: "Predicted end of period",
                    flow: fillHeight,
                    day: periodEnd.getDate(),
                    month: periodEnd.getMonth(),
                    year: periodEnd.getFullYear(),
                  },
                ]);
                break;
              case "End of period":
                // logic to update database info for length of period - get rid of any predicted dates etc
            ...
            <View>
              ...
              <View style={styles.sliderContainer}>
                <View style={styles.circle}>
                  <View style={[styles.circlefill, { height: fillHeight }]} />
                  <Text style={styles.flowText}>{flowRate()}</Text>
                </View>
                <Slider
